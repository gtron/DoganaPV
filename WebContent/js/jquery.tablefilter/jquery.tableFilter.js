    ...
    prototype: {
        ...
        parseTable: function(){
            ...
            if (this.settings.paging && this.tbody.rows.length){
                ...
                this.pagingRow.append('<td colspan="'+this.cellCount+'" align="left" nowrap="nowrap" valign="middle"><div id="p_base"> <img id="pageFirst" src="'+ip+'/page-first-disabled.gif" align="absmiddle" alt=""> <img id="pagePrev" src="'+ip+'/page-prev-disabled.gif" align="absmiddle" alt="">  Pagina <input id="pageCurrent" size="2" value="1" type="text"> of <span id="pageCount">'+this.maxPage+'</span> <img id="pageNext" src="'+ip+'/page-next.gif" align="absmiddle" alt=""> <img id="pageLast" src="'+ip+'/page-last.gif" align="absmiddle" alt=""> <img src="'+ip+'/toolbar.gif" align="absmiddle" alt=""> Visualizza: <input id="pageRecords" size="2" value="'+this.settings.pageLength+'" type="text"> righe per pagina - (<span id="pageVisible"></span> visibili/<span id="pageTotal"></span> totali) <img src="'+ip+'/toolbar.gif" align="absmiddle" alt=""> <a id="p_menu_trigger">&nbsp;Menu</a></div></td>')
				...
				/*---ANDREA------DISABILITO IL MENU PERCHE NON MI FUNZIONA
            ...
            // set up colgorup for column locking
        ...
        parse:{
            ...
            date: function(tmpCol,text){
            	...
            	//ANDREA-----------LA DATA INSERITA IN FORMATO YYMMDD LA TRASFORMO NEL FORMATO YYYYMMDD
            	var _ddmmyyDateTofullDate = function(fullDate){
            		var tmp_date=fullDate.split('/');
            		return '20' + tmp_date[2] + '/'+ tmp_date[1] + '/'+tmp_date[0];
            	};
            	//---------------------------------------------------------------------------------
                ...
                var _unique = function (month,year,day){
                    if (tmpCol.uniqueDays.length < 31 && tmpCol.uniqueDays.indexOf(day) < 0){
                        tmpCol.uniqueDays.push(day);
                    };
             ...
             //  tmp = Date.parse(text);
                try{
                	...
                	text=_ddmmyyDateTofullDate(text); 
                	tmp = Date.parse(text);
                	}
                	catch(e){alert(e.message); }
        ...
        buildColumns: function() {
            ...
            jQuery.each(root.cols,function(i,v){
                ...
                if (v ){
					v.buildSort(sRow[i]);
					if ((v.dataType == "date" && v.uniqueDays.length > 1 && v.uniqueMonths.length > 1) && (v.dataType == "date" && v.uniqueMonths.length > 1) || (v.dataType != "date" && v.uniqueText.length > 1)){
    ...
    prototype : {
        ...
        getFilter: function(){
            switch(this.dataType){
                ...
                case "date":
                return function(arg){
                    var ff, ffml, ffdl,ffyl, m, y,d;
                    if (arg[0] === null || !arg[0].length){
                        return false;
                        } else {
                        ff = this.filterField;
                        //alert(ff[0].value + ' - ' + ff[1].value + ' '+ ff[2].value + ' ' + arg[0]);
                        ffdl = ff[0].value.length;
                        ffml = ff[1].value.length;
                        ffyl = ff[2].value.length;
                        d = (ffdl && ff[0].value == arg[0].substr(6,2));
                        m = (ffml && ff[1].value == arg[0].substr(4,2));
                        y = (ffyl && ff[2].value == arg[0].substr(0,4));
                        return ((ffml && ffyl && ffdl && m & y & d) 
                        || !ffdl && ((!ffyl && ffml && m) || (!ffml && ffyl && y) || (ffml && ffyl && m & y))
                        || !ffyl && (ffml && ffdl && d & m) || (!ffml && ffdl)
                        );
                    };
                };
                break;
        ...
        buildFilter: function (ele){
            ...
            switch (this.dataType){
                ...
                this.addField(ele,"select",this.uniqueDays);
